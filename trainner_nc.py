...
class NCTrainer(object):
        ...
        with tqdm(range(1, args.max_epoch + 1)) as bar:
            for epoch in bar:
                ...
                if epoch == 1 or epoch % self.args.log_interval == 0:
                    ...
                    print(f"Every Test: Test15:{best_test_acc_list[0]:.4f}, "
                          f"Test16: {best_test_acc_list[1]:.4f}, Test17: {best_test_acc_list[2]:.4f}")
    ...
    def train(self, epoch, data):
        ...
        embedding_list = self.model(data['edge_index'], self.x[:self.len_train], self.len_train)
        ...
        for t in range(self.len_train):
            ...
            pred = self.cal_pred(z, self.model.cs_decoder,
                                 data['node_masks'][t].to(self.args.device))
            step_loss = criterion(
                pred, data['y'][data['node_masks'][t]].squeeze().to(self.args.device))
        ...
        embedding_list = self.model(data['edge_index'], self.x, self.len)
        for t in range(self.len):
            z = embedding_list[:, t, :].squeeze()
            acc = self.predict(z, self.model.cs_decoder,
                               data['node_masks'][t], data['y'])
            if t < self.len_train:
